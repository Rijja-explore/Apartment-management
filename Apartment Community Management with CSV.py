...
def check_res():
    ...
    if (y is not None and user_input == y[0] and y[0] != '' and code_input == y[1]) or (
            z is not None and user_input == z[0] and z[0] != '' and code_input == z[1]):
        return res_Profile()
    else:
        return error()
...
def resident():
    ...
    code = Entry(frame, width=35, fg='black', border=2, bg='white', font=('Microsoft YaHei UI light', 11),
                 textvariable=password,show="*")
    code.place(x=30, y=180)
    Button(frame, width=18, pady=7, text='Forgot Password?', bg='#57a1f8', fg='white', border=0, command=forgot_pwd).place(x=180,y=250)
    Button(frame, width=18, pady=7, text='Sign In', bg='#57a1f8', fg='white', border=0, command=check_res).place(x=35,y=250)
                                                                                                    
    root.mainloop()

def forgot_pwd():

    FlatNo = askstring('Password Reset','Enter your Flat Number: ')

    L = [x[4] for x in Magenta if x[0] != '']
    L.extend([x[4] for x in Turquoise])

    if FlatNo not in L:
        messagebox.showerror("Invalid Flat No", "Make sure to enter your exact Flat No")

    else:
        email_sender = 'SpikNSpan.noreply@gmail.com'
        email_password = "iohxekqubaluupwx"
        email_receiver = 'spiknspan2024@gmail.com'  #Enter the receiving email here

        subject = "Password Reset"

        body = f''' Hi Tariq,

This email is to inform you that the resident of the Flat No: {FlatNo} requires a password reset for their account on the community portal .
Please reset their password at your earliest convenience'''
        ...
        messagebox.showinfo('Password Reset', 'Your details have been forwarded.The adminstrator will reset the password shortly')
